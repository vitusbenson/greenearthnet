...
def predict_prevseason(minicube_path, targ):

    minicube = xr.open_dataset(minicube_path).sel(
        time=slice(
            targ.time.values[0] - np.timedelta64(465, "D"),
            targ.time.values[0] - np.timedelta64(115, "D"),
        )
    )
    ...
    try:
        ...
        pred = (
            xr.DataArray(
                minicube.s2_ndvi.interpolate_na("time", method="linear")
                .clip(-1, 1)
                .fillna(0.5)
                .sel(
                    time=slice(
                        targ.time.values[0] - np.timedelta64(365, "D"),
                        targ.time.values[0] - np.timedelta64(216, "D"),
                    )
                )
                .values,
            ...
            .isel(time=slice(54, None, 5))
            .to_dataset(name="ndvi_pred")
        )
...
def score_from_args(args):
    ...
    pred = predict_prevseason(full_path, targ)
