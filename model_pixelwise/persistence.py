...
def predict_persistence(minicube_path, pred_path):
    ...
    minicube["s2_ndvi"] = ((nir - red) / (nir + red + 1e-8)).where(
        ((minicube.s2_dlmask < 1) & minicube.s2_SCL.isin([1, 2, 4, 5, 6, 7])), np.NaN
    )

    ndvi_arr = (
        minicube.s2_ndvi.isel(time=slice(4, None, 5))
    ...
    pred_ndvi = ndvi_arr[None, ...].repeat(20, 0)
    ...
    pred_cube = xr.Dataset(
        {
            "ndvi_pred": xr.DataArray(
                data=pred_ndvi,
                coords={
                    "time": minicube.time.isel(time=slice(4, None, 5)).isel(
                        time=slice(10, 30)
                    ),
                    "lat": lat,
                    "lon": lon,
...
def score_from_args(args):
    ...
    targetfile, pred_dir, name_ndvi_pred = args
    ...
    pred = predict_persistence(targetfile, pred_path)
...
def score_over_dataset(
    ...
    for region in (
        ...
        for targetfile in targetfiles:
            inputargs.append([targetfile, pred_dir, name_ndvi_pred])
